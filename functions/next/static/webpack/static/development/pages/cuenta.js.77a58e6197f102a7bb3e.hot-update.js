...
const useLogin = (type, email, pass, err) => {
  if (type === true) return _credentials_servers__WEBPACK_IMPORTED_MODULE_1__["default"].auth().createUserWithEmailAndPassword(email, pass).catch(err);else if (type === false) return _credentials_servers__WEBPACK_IMPORTED_MODULE_1__["default"].auth().signInWithEmailAndPassword(email, pass).catch(err);else if (type === "fb") return _credentials_servers__WEBPACK_IMPORTED_MODULE_1__["default"].auth().signInWithRedirect(fbprovider).catch(err);else if (type === "g") return _credentials_servers__WEBPACK_IMPORTED_MODULE_1__["default"].auth().signInWithRedirect(gprovider).catch(err);
};
...
//# sourceMappingURL=cuenta.js.77a58e6197f102a7bb3e.hot-update.js.map