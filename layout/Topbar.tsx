...
let renderManager = (count: number) => count
...
const TopBar: React.FC<Props> = (props: Props, ref: any) => {
	...
	useEffect(() => {
		...
		const addToCartBtn: HTMLButtonElement | null = document.querySelector(
			'.addToCartBtn'
		) as HTMLButtonElement
		const shopCount: HTMLButtonElement | null = document.getElementById(
			'shopCount'
		) as HTMLButtonElement
		...
		const addShadow = () => {
			if (window.scrollY > 15 && topbar) topbar.style.boxShadow = '0 0 10px rgba(0,0,0,.4)'
