...
//  PROPIEDADES
interface InfoProps { updateCall: Function; user: userModel | firestore.DocumentData, strings: langPackage.profilePage["info"] }

// COMPONENTE
const ProfileInformation: React.FC<InfoProps> = (props: InfoProps) => {
  // REFERENCIAS
  const defForm: IForms = { displayName: props.user.displayName || "", email: props.user.email || "", address: props.user.address, phone: props.user.phone?.toString(), nit: props.user.nit };
  ...
  const saveToForm = (data: InputGetProps) => {
    ...
    if (values.current) Object.keys(values.current).forEach((keys: string) => {
      // @ts-ignore    
      if (values.current && keys === data.name) values.current[keys] = data.text;
      if (values.current && data.name === "nit" && data.text.length === 0) values.current.nit = "C/F";
    })
  }
  ...
  const verifyUpdate = (defValue: IForms | userModel | firestore.DocumentData, newValue: IForms) => {
    return (defValue.address === newValue.address &&
  ...
  useEffect(() => {
    ...
    const updateInterval = setInterval(() => {
      if (updateBtn.current && values.current && navigator.onLine && values.current.phone.toString().length === 8 && !verifyUpdate(props.user, values.current)) {
  ...
  return (
    <div id="profileInfo">
      <h2>{props.strings.title} <i className="material-icons">assignment</i></h2>
      ...
      <div id="updateSection">
        <button onClick={sendToUpdate} ref={updateBtn} className="waves gray updateInfo">{props.strings.button}</button>
      </div>
      <style jsx>{`
        p{
          margin-bottom:30px;
